...
- [x] 搜索并读取 AI 定义、AI 能力框架、大语言模型机制、Prompt、Skill、Agent/RAG、评测和参数定制的一手资料。
- [x] 对关键结论进行交叉核对，记录来源的时效性和厂商特定边界。
- [x] 编写研究综述，明确事实、分析判断和课程建议。
- [x] 建立完整来源台账并检查每个正文引用均有对应条目。
...
- [x] 按六个时间段编写大纲，确保每段都有“听众带走什么”。
- [x] 编写 4,200–4,800 中文字符的讲稿，保持单一案例递进并控制术语数量。
- [x] 对事实、类比、教学建议和模拟材料做明确标识。
- [x] 运行估时脚本并人工检查开场、互动、转场和收束是否在预算内。
...
- [x] 编写主案例原始材料和模糊 Prompt，明确全部为教学模拟。
- [x] 用五字段逐层改写并解释每一字段解决的失败模式。
- [x] 编写三个迁移案例，禁止加入未经来源支持的联想内部事实。
- [x] 检查每个完整 Prompt 是否含目标、上下文、约束、输出和验收/停止条件。
...
- [x] 编写 Skill 的触发描述、输入、步骤、资源读取、异常、停止条件和输出规范。
- [x] 编写独立输出合同，说明事实来源与人工确认边界。
- [x] 编写仅使用标准库的检查脚本，对 Markdown 表头、列数、空值策略和禁止猜测词进行校验。
- [x] 分别运行合格样例和故意缺字段样例，记录可复现结果。
...
- [x] 设计 10 页内容页加 1 页来源页，逐页标注时间和讲稿映射。
- [x] 每页只保留一个完整结论，并给出对比图、流程图或决策表建议。
- [x] 编写讲师指南，覆盖术语发音、提问方式、可能误解和删减顺序。
- [x] 检查页纲没有把讲稿复制成密集项目符号。
...
- [x] 编写 README，给出授课、快速预览、制作 PPT 和运行检查的方法。
- [x] 编写仓库检查与估时脚本，输出明确通过项和失败原因。
- [x] 运行全部脚本、检查 Markdown 链接、核对来源映射并保存结果。
- [x] 完成自检报告，逐项记录通过、限制和正式授课前行动。
- [x] 执行 `git diff --check`、检查仓库状态、提交完整教材并确认工作区干净。
